...
def scrape_models(task):
    ...
    r = s.get(query_url, proxies=proxies, timeout=settings.REQUEST_TIMEOUT)
    if r.status_code == 200:
        raw_data = r.json()
...
def scrape_auto_scout24_listings(task):
    ...
    try:
        r = s.get(
            req_url, proxies=proxy_utils.get_proxy(), timeout=settings.REQUEST_TIMEOUT
        )
        ...
        logger.error(
            f"Failed to get request url: {req_url}. uid: {uid}. error: {str(e)}"
        )
        return
